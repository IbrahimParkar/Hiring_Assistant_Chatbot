...
# Candidate Information Form - Show only if form is not submitted
if st.session_state.show_form and "form_submitted" not in st.session_state:
    ...
    if proceed_clicked:
        if not is_valid_email(email):
            ...
            # All good: Save profile and hide form
            ...
            st.session_state.form_submitted = True  # Hide the form after submission
...
# Interview Questions - Only show after form is submitted
    ...
    if current_q_idx < len(st.session_state.questions):
        ...
        if current_q_idx == total_questions - 1 and complete_button:
            ...
            if not cleaned_response.strip():
                ...
                # Step 1: Relevance check before storing
                is_relevant = is_response_relevant(
                    st.session_state.questions[current_q_idx],
                    cleaned_response
                )
